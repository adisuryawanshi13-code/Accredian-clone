...
import { referralSchema } from "../../validations/referral.schema";
import { ApiResponse } from "@/shared/types/common.types";
import { ReferralSuccessData } from "../../types/referral.types";

export class ReferralServerService {
  /**
   * Processes an incoming referral POST payload and validates inputs.
   */
  public async processReferral(
    request: Request
  ): Promise<NextResponse<ApiResponse<ReferralSuccessData>>> {
    try {
      ...
      const errorMessage =
        err instanceof Error
          ? err.message
          : "Something went wrong while processing referral";
...
export const referralServerService = new ReferralServerService();
